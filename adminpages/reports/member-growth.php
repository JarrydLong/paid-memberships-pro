...
$pmpro_reports['member_growth'] = __('Member Growth', 'paid-memberships-pro' );


//queue Google Visualization JS on report page
function pmpro_report_member_growth_init()
{
	if ( is_admin() && isset( $_REQUEST['report'] ) && $_REQUEST[ 'report' ] == 'member_growth' && isset( $_REQUEST['page'] ) && $_REQUEST[ 'page' ] == 'pmpro-reports' ) {
		wp_enqueue_script( 'corechart', plugins_url( 'js/corechart.js',  plugin_dir_path( __DIR__ ) ) );
	}

}
add_action("init", "pmpro_report_member_growth_init");

//widget
function pmpro_report_member_growth_widget() {
	global $wpdb;
?>
<style>
...
<span id="pmpro_report_sales" class="pmpro_report-holder">
	...
	foreach ( $reports as $report_type => $report_name ) {
		...
		$prices = pmpro_get_prices_paid1( $report_type, $max_prices_count );
		?>
		<tbody>
			<tr class="pmpro_report_tr">
				...
				<td><?php echo esc_html( number_format_i18n( pmpro_getSales1( $report_type, null, 'all' ) ) ); ?></td>
				<td><?php echo pmpro_escape_price( pmpro_formatPrice( pmpro_getRevenue1( $report_type ) ) ); ?></td>
			</tr>
			<?php
				//sale prices stats
				$count = 0;
	...
	<?php if ( function_exists( 'pmpro_report_member_growth_page' ) ) { ?>
		<p class="pmpro_report-button">
			<a class="button button-primary" href="<?php echo esc_url( admin_url( 'admin.php?page=pmpro-reports&report=member_growth' ) ); ?>"><?php esc_html_e('Details', 'paid-memberships-pro' );?></a>
		</p>
	<?php } ?>
</span>
...
function pmpro_report_member_growth_page() {


	global $wpdb, $pmpro_currency_symbol, $pmpro_currency, $pmpro_currencies, $pmpro_levels;
var_dump('xxx');
...
var_dump($dates);
	...
	if( $period === '7days' || $period === '30days' || $period === '12months' ) {
		...
		for( $i = 1; $i <= $lastday; $i++ ) {
			...
			$cols[$i] = array(0, 0);
			$csvdata[$i-1] = (object)array('date'=>$i, 'total'=>'', 'new'=> '', 'renewals'=>'', 'level_id' => '' );
			if ( ! $currently_in_period || $i < $day_of_month ) {
				$units_in_period++;
			}
	...
	elseif($period == "monthly")
	{
		...
		for($i = 1; $i < 13; $i++)
		{
			//here
			$cols[$i] = array(0, 0);
			$csvdata[$i-1] = (object)array('date'=>$i, 'total'=>'', 'new'=> '', 'renewals'=>'', 'level_id' => $date->level_id );
			...
			foreach($dates as $date)
			{
				if($date->date == $i) {
					$cols[$i] = array( $date->value, $date->renewals );
					$csvdata[$i-1] = (object)array('date'=>$i, 'total'=>$date->value, 'new'=> $date->value - $date->renewals, 'renewals'=> $date->renewals, 'level_id' => $date->level_id);
					if ( ! $currently_in_period || $i < $month_of_year ) {
						$total_in_period += $date->value;
					}
				}
	...
	foreach( $dates as $date ) {
		if( ! isset( $date->month ) ) {
			$grouped_month[$date->month] = array( $date );
		} else {
			$grouped_month[$date->month][] = $date;	
		}
		
	}

	var_dump($grouped_month);
	/**
	 * Group them into dates first
	 * Then get a total for that level
	 * each level gets a series
	 */
	...
	<div class="tablenav top">
		...
		<input type="hidden" name="report" value="member_growth" />
	...
	<script>
		...
		function drawVisualization() {
			...
			var dataTable = google.visualization.arrayToDataTable([
				<?php 
					$level_amounts = $level_names = array();
					$level_names[] = "Date";
					foreach( $pmpro_levels as $level ) {
						$level_names[] = $level->name;
					}
					echo wp_json_encode(  $level_names  );

				?>,
				<?php 
				$chart_data = array();
				foreach( $level_names as $ln ) {
...
//get sales
function pmpro_getSales1( $period = 'all time', $levels = 'all', $type = 'all' ) {	
 ...
 */
function pmpro_get_prices_paid1( $period, $count = NULL ) {
	// Check for a transient.
...
//get revenue
function pmpro_getRevenue1( $period, $levels = NULL, $type = 'all' ) {
	//check for a transient
 ...
 */
function pmpro_get_revenue_between_dates1( $start_date, $end_date = '', $level_ids = null ) {
	global $wpdb;
...
function pmpro_report_member_growth_delete_transients1()
{
	delete_transient( 'pmpro_report_sales' );
	delete_transient( 'pmpro_report_revenue' );
	delete_transient( 'pmpro_report_prices_paid' );
}
add_action("pmpro_after_checkout", "pmpro_report_member_growth_delete_transients1");
add_action("pmpro_updated_order", "pmpro_report_member_growth_delete_transients1");
add_action("pmpro_added_order", "pmpro_report_member_growth_delete_transients1");